...
One real query-time smoke test is still outstanding: PostgreSQL is healthy and the host Ollama HTTP endpoint responds, but the API container's configured `host.docker.internal:11434` connection was refused during the query. The local index and copied vectors are present; container-to-Ollama reachability must be fixed and then retrieval latency/quality benchmarked before calling the service end-to-end ready. Wikidata's 906 unavailable original snapshots and their explicitly linked assertion impact remain unresolved and must not be silently folded into this narrative index.
